...
def list_melons():
    ...
    ###Using the get_all method in melons.py, created a list of values
    ###This list is formated by the __repr__ function in melons.py
    return render_template("all_melons.html",
                           melon_list=melon_list)
                            ###Orange side is the Jinja variable
                            ###We're building the page everytime we go to melons
                            ####In Jinja, we unpack each of the relevant parts 
                            ###in the for loop. 


@app.route("/melon/<melon_id>")
###The decorator allows <melon_id> to be passed in as an argument in the
###show_melon parameter
...
#The melon_id is passed in from the html
def add_to_cart(melon_id):
    ...
    if 'cart' in session:
        ...
        cart = session['cart'] = {}
    ###Session and cart are both dictionaries
    ...
    #Checking to see if elements are in the cart dictionary

    ###ALTERNATIVE TO cart[melon_id] = cart.get(melon_id, 0) + 1
    ### if cart[melon_id]: 
    ...
    ###This generates the flash message

    # Redirect to shopping cart page
    return redirect("/cart")
    ###Flash msg won't go away unless you redirect
